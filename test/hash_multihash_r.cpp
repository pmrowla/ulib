...
int main()
{
	multi_hash_map<str, int> months(128);
	...
	multi_hash_map<uint64_t, int> map(1024);
	...
	multi_hash_map<uint64_t, uint32_t, ulib_except, combiner> wc(128);
	...
	for (multi_hash_map<uint64_t, uint32_t, ulib_except, combiner>::iterator it = wc.begin();
	     it != wc.end(); ++it) {
		printf("key:%lu, value: %u, hid=%zu, size=%zu, bucket=%zu\n",
		       (unsigned long)it.key(), it.value(), it._hid, it._ht[it._hid].size(), it._nht);
		++size;
	}
