...
int main()
{
    ...
    assert(chainhash_key(it) == 1);
    assert(chainhash_value(it) == 2);
    it = chainhash_set_uniq(my, ch, 1, 3, 1);
    assert(!chainhash_end(it));
    assert(chainhash_key(it) == 1);
    assert(chainhash_value(it) == 3);
    it = chainhash_get(my, ch, 1);
    assert(!chainhash_end(it));
    assert(chainhash_key(it) == 1);
    assert(chainhash_value(it) == 3);
    it = chainhash_set_uniq(my, ch, 2, 1, 1);
    assert(!chainhash_end(it));
    assert(chainhash_key(it) == 2);
    assert(chainhash_value(it) == 1);
    it = chainhash_get(my, ch, 2);
    assert(!chainhash_end(it));
    assert(chainhash_key(it) == 2);
    assert(chainhash_value(it) == 1);
    it = chainhash_set_uniq(my, ch, 2, 2, 0);
    assert(!chainhash_end(it));
    assert(chainhash_key(it) == 2);
    assert(chainhash_value(it) == 1);
    it = chainhash_get(my, ch, 2);
