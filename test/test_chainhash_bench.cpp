...
DEFINE_CHAINHASH(myhash, uint64_t, uint64_t, chainhash_hashfn, chainhash_cmpfn)
...
void constant_insert(long ins, long get)
{
    ...
    chainhash_t(myhash) *my = chainhash_init(myhash, ins * 5/4);
    ...
    for (t = 0; t < ins; t++) {
        chainhash_itr_t(myhash) itr =
                chainhash_set(myhash, my, myrand(), 1);
        if (!chainhash_end(itr))
            chainhash_value(itr) = t;
        counter++;
    }
